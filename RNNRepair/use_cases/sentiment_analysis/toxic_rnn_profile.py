...
from .toxic_datasets import TOXIC, TOXIC_LST
...
class TOXICClassifier(Profiling):
    ...
    def load_default_data(self):
        train_lst = joblib.load(os.path.join(self.sen_path, 'toxic_train.lst'))
        test_lst = joblib.load(os.path.join(self.sen_path, 'toxic_test.lst'))
    ...
    def train(self, dataset = None, is_load = False, saved_path = None):
        ...
        train_data, test_data = TOXIC_LST.splits(self.TEXT, self.LABEL, root='npy', **para)
    ...
    def eval_test(self, model, x_test, save_text=None, save_name = '', need_loss = False):
        ...
        test_data = TOXIC_LST.splits(self.TEXT, self.LABEL, root='npy', **para)
    ...
    def predict(self, model, iterator, save_text=False, save_name=None, need_loss = False): # n * 28 * 28
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(iterator):
                ...
                softmax =  TOXICClassifier.ut_convert_1dim_to_2dim_aftersigmoid(softmax_1dim)
